...
- **繁體中文 是两个问题，也就当两个问题处理。** 字形折叠是机械的（随包发布一张
  ...
  过折叠的结果。
- **主诉和诊断落在 ICPC-3 上。** `resolve_symptom("头疼")` 得到 `NS01`，
  `resolve_condition("高血压")` 得到 `KD73`：ICPC-3 的 S、D 两个组件共 1,218 个码原样随包，
  中英文说法是我们自己写的别名。两套索引分开，主诉不会落到诊断上；拿不准就弃答并给原因，不猜。
  在 200 条真实问诊里的主诉上测过，金标准由三个互相独立的模型标注（三种不同的模型，看不到解析器）：
  可计分的 142 条症状里没有一个错码，编上码的约五分之一，其余弃答。
- **单位**按 328 个 UCUM 单位归一到 59 个 PROPERTY 家族，带量纲分析，
