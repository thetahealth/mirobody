...
Usage:
    python3 -m mirobody.pulse.aggregate.test_cgm_indicators
"""
...
async def test_db_aggregation():
    ...
    # Integration test: needs a configured database holding the reference user's
    # data. On a bare checkout there is no config at all — skip honestly instead
    # of failing with "no configuration found" on every machine.
    if global_config() is None:
        pytest.skip("requires a configured database with reference data (integration test)")
