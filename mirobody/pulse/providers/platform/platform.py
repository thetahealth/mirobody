...
class ProviderPlatform(Platform):
    ...
    def get_provider(self, provider_slug: str) -> Optional[BasePullProvider]:
    ...
    def load_providers(self) -> List[BasePullProvider]:
        ...
        for directory in all_dirs:
            logging.info(f"Scanning for providers in: {directory}")
    ...
    async def stop_pull_scheduler(self) -> None:
        try:
            ...
            logging.info("provider pull scheduler stopped successfully")
        except Exception as e:
    ...
    async def get_webhooks(
        ...
        logging.info(f"Getting webhooks for provider {provider}: page={page}, page_size={page_size}, user_id={user_id}")
    ...
    async def check_format(self, webhook_id: int, provider: Optional[str] = None) -> Dict[str, Any]:
        ...
        logging.info(f"Checking format for provider {provider}, webhook_id: {webhook_id}")
