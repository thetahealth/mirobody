    ...
    def _process_sleep_data(self, data: List[Dict], processing_info: Dict) -> List[StandardPulseRecord]:
        ...
        for sleep in data:
            try:
                ...
                timestamp_ms = (
                    TimeUtils.parse_time_to_timestamp(start_time_str) if start_time_str else int(time.time() * 1000)
                )
    ...
    def _process_recovery_data(self, data: List[Dict], processing_info: Dict) -> List[StandardPulseRecord]:
        ...
        for recovery in data:
            try:
                ...
                timestamp_ms = (
                    TimeUtils.parse_time_to_timestamp(created_at_str) if created_at_str else int(time.time() * 1000)
                )
