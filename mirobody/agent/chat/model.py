...
class ChatStreamRequest:
    ...
    **As of 1.4.1 the shipped `frontend/` bundle sends none of them** — the web
    client was rebuilt in the same release. They still stay, for one release,
    because a browser holding a cached older bundle keeps sending `agent` and
    `group_id`, and the mobile client is not this repository's to rebuild. Drop them once neither is in the wild; that
