...
def load_agents_from_module(module: ModuleType, module_name: str, config: Config = None) -> dict:
    ...
    for class_name, klass in classes:
        ...
        # Assigned only when generate_response is found. Without the guard
        # below, a class defining load_llm_clients but no generate_response
        # would NameError here — it never did only because inspect.getmembers
        # returns members alphabetically and "generate_response" happens to
        # sort before "load_llm_clients".
        agent_name = ""
        ...
        if agent_name and config and callable(load_llm_clients_func):
...
def agent_keeps_own_history(agent_name: str) -> bool:
    """Whether an agent persists its own conversation history.

    DeepAgent-family agents (DeepAgent / AppAgent) compile their graph with a
    LangGraph checkpointer keyed on ``thread_id = session_id``, so LangGraph
    stores the real AIMessage/ToolMessage objects and replays them itself. The
    caller must hand such an agent ONLY the new turn — passing replayed history
    as well would duplicate every message.

    BaseAgent has no graph and no checkpointer, and its stateless providers
    (DeepSeek) get nothing from the provider side either, so it still needs the
    caller's flat {role, content} replay.

    This used to be ``agent_uses_canonical_history`` and answered a different
    question — "can this agent digest BaseMessage objects?" — back when the
    caller rebuilt LangChain messages out of persisted UI chunks. The callers
    are the same; what they do with the answer is now the opposite.

    Resolution failures default to False (send history), the safe direction: a
    duplicated turn reads worse than a missing one, but a silently amnesiac
    agent is the failure nobody notices.
    """
    try:
        ...
        logging.warning(f"agent_keeps_own_history({agent_name}) failed: {e}; will replay history")
        return False
