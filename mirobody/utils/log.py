...
@deprecated(reason="use logging package instead")
def log_setter(
    _input: dict | str = "",
    level: str = "info",
    **kargs
    ...
    for key in kargs:
        ...
        if key == "call_depth":
            stacklevel = kargs[key]

        elif key == "cost_time":
            extra["time_cost"] = round(kargs["cost_time"], 4)

        else:
            extra[key] = kargs[key]

    #-----------------------------------------------------
    # Fields residing in the request context.
