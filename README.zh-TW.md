...
- **單位標準化**成 UCUM 家族（約 310 個），再加上[單位轉換](mirobody/indicator/fhir/units/convert.py)——因次分析、依 LOINC 代碼查表的摩爾質量橋接，以及對 `%` 相對 `10*9/L` 這種情況明確拒絕轉換。300 個標準 pulse 指標。
- 5 個根類別（人口統計、身體量測、生命徵象、生活型態與社會、檢驗與臨床）共 25 個
  標籤的分類法——「檢驗與臨床」底下的 11 個身體系統是它的子層，不是同層。
